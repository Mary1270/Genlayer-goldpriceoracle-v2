...
logic, this stub reproduces just enough of the SDK's surface area to
import and exercise `contract.py` directly in plain Python, with
...
class _EqPrinciple:
    ...
    The contract uses `prompt_comparative` (not `strict_eq`) for its
    fetch+LLM pipeline, per GenLayer's documented guidance that
    strict_eq must never be used for LLM-derived output. For offline
    unit tests we simply run `fn` once and return its result.
    """
